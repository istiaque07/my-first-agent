...
LLM_CONFIG = {
    ...
    "g4f": {
        "api_keys": [],  # g4f uses local package providers; no remote API key is required
        "url": "",
        "model": "gpt-4o-mini"
    },
    "groq": {
...
LLM_PROVIDER_ORDER = list(LLM_CONFIG.keys())
DEFAULT_LLM_PROVIDER = "g4f" # Set your preferred default: "g4f", "gemini", "huggingface", or "groq"
...
DMM_MODEL_ORDER = [
    "command", "command-medium", "command-large", "command-light",
    "command-medium-nightly", "command-large-nightly", "command-xlarge-nightly"
]


def classify_with_general_llm(prompt: str):
    """
    Uses the configured general LLM providers to classify a user prompt if Cohere DMM is unavailable.
    """
    print("Falling back to general LLM classification because Cohere DMM failed.")
    chat_history = load_chat_history()
    classification_prompt = (
        "You are a command classifier for an AI assistant. "
        "Classify the user command below into one of these exact formats: "
        "'general (query)', 'realtime (query)', 'open (target)', 'close (target)', "
        "'play (query)', 'generate image (query)', 'system (task)', 'content (topic)', "
        "'google search (query)', 'youtube search (query)', or 'reminder (details)'. "
        "Do not answer the question, only return the classification in the specified format. "
        "If the command cannot be classified, respond with 'general (command)'.\n\n"
        f"User command: {prompt}"
    )

    response_text, used_provider = call_llm_api_with_fallback(DEFAULT_LLM_PROVIDER, chat_history, classification_prompt)
    if not response_text or (isinstance(response_text, str) and response_text.startswith("ERROR_RESPONSE:")):
        print(f"General LLM classification fallback failed: {response_text}")
        return []

    response_text = response_text.replace("\n", " ").strip()
    classified_commands = [i.strip() for i in response_text.split(",") if i.strip()]
    filtered_commands = []
    for task in classified_commands:
        normalized_task = task.strip()
        for func in DMM_FUNCS:
            if normalized_task.lower().startswith(func):
                filtered_commands.append(normalized_task)
                break
...
def FirstLayerDMM(prompt: str = "test"):
    """
    Uses Cohere's command-style model to classify the user's prompt into a predefined function.
    """
    ...
    if cohere_client is None:
        ...
        return classify_with_general_llm(prompt)
    ...
    try:
        ...
        response_text = ""
        used_model = None
        for model_name in DMM_MODEL_ORDER:
            try:
                stream = cohere_client.chat(
                    model=model_name,
                    message=prompt,
                    temperature=0.7,
                    chat_history=current_dmm_history_for_api,
                ...
                if model_response:
                    ...
                    print(f"DMM model {model_name} returned empty response. Trying next model.")
            except Exception as e:
                print(f"DMM model {model_name} failed: {e}. Trying next model.")
                continue

        if not response_text:
            print("Warning: No DMM model produced a valid response.")
            return []
        ...
        if not response_text:
            response_text = f"general (DMM could not classify: {prompt})"
...
def call_llm_api(provider, chat_history_for_llm, user_query_for_llm, image_data_base64=None):
    ...
    if provider != "g4f" and not api_keys:
    ...
    if provider == "g4f":
        g4f_messages = [{"role": "system", "content": system_instruction}]
        for msg in chat_history_for_llm:
            role = "user" if msg.get('sender') == 'user' else "assistant"
            g4f_messages.append({"role": role, "content": msg.get('text', '')})
        g4f_messages.append({"role": "user", "content": user_query_for_llm})
        ...
        try:
            ...
            print(f"g4f provider failed: {e}")
            return f"ERROR_RESPONSE: g4f provider error: {e}"
    ...
    for key in api_keys:
        ...
        try:
            ...
            return f"ERROR_RESPONSE: {provider} connection error."
        except requests.exceptions.Timeout as errt:
            print(f"API Key {key[:5]}... for {provider}: Timeout: {errt}")
            return f"ERROR_RESPONSE: {provider} timeout error."
        except requests.exceptions.RequestException as err:
            print(f"API Key {key[:5]}... for {provider}: Something went wrong: {err}")
            return f"ERROR_RESPONSE: {provider} unknown request error."
...
def call_llm_api_with_fallback(primary_provider, chat_history_for_llm, user_query_for_llm, image_data_base64=None):
    """Calls LLM providers in order and automatically switches if the primary provider fails."""
    provider_order = [primary_provider] + [p for p in LLM_PROVIDER_ORDER if p != primary_provider]
    last_error = None

    for provider in provider_order:
        response = call_llm_api(provider, chat_history_for_llm, user_query_for_llm, image_data_base64)
        if not response or (isinstance(response, str) and response.startswith("ERROR_RESPONSE:")):
            last_error = response
            print(f"Provider {provider} failed. Trying next provider. Error: {response}")
            continue
        return response, provider

    fallback_text = f"Sir, all LLM providers failed. Last error: {last_error}" if last_error else "Sir, all LLM providers failed."
    return fallback_text, primary_provider
...
def set_alarm_or_reminder(command_text):
    ...
    Returns True if the command was recognized and successfully scheduled,
    False otherwise.
    """
            ...
            for fmt in time_formats:
                try:
                    ...
                    if "p" not in fmt.lower() and "a" not in fmt.lower() and parsed_time.hour < 12:
                        if alarm_datetime < now and (now - alarm_datetime).total_seconds() > 3600:
                            alarm_datetime += datetime.timedelta(hours=12)
    ...
    reminder_time_str = (datetime.datetime.now() + datetime.timedelta(seconds=time_delta_seconds)).strftime("%I:%M %p")
    speak_response(f"Certainly, sir. I will remind you at {reminder_time_str} about {message}.")
...
def _perform_raw_google_search(query):
    ...
    try:
        ...
        if not results:
            return f"Sir, I could not find search results for '{query}'."
        ...
        for i, res in enumerate(results, start=1):
            if isinstance(res, str):
                ...
                title = getattr(res, 'title', None) or getattr(res, 'name', None) or ''
                description = getattr(res, 'description', None) or getattr(res, 'snippet', None) or ''
                url = getattr(res, 'url', None) or getattr(res, 'link', None) or ''
                Answer += f"{i}. {title}\n{description}\n{url}\n\n"
        return Answer
    except Exception as e:
...
def perform_chrome_search(command_text, query_from_dmm=None, llm_provider=None, provider_store=None):
    """
    ...
    if query_from_dmm: # If DMM already extracted the query
        ...
        provider_to_use = llm_provider if llm_provider else DEFAULT_LLM_PROVIDER
        response_from_llm, used_provider = call_llm_api_with_fallback(provider_to_use, current_chat_history, llm_prompt)
        if provider_store is not None and used_provider != provider_to_use:
            provider_store[0] = used_provider
            speak_response(f"Sir, I have automatically switched to {used_provider} because {provider_to_use} failed.")
...
def handle_realtime_query(prompt_from_dmm, chat_history_main):
    ...
    response_from_llm, used_provider = call_llm_api_with_fallback("groq", chat_history_main, llm_prompt)

    if used_provider != "groq":
        speak_response(f"Sir, I am using {used_provider} instead of Groq because Groq was unavailable.")
...
def main():
    ...
    while True:
        ...
        if command:
            ...
            # --- Quick direct commands handled before the DMM ---
                ...
                if len(parts) > 1:
                    ...
                    if provider_name in LLM_CONFIG:
                        if LLM_CONFIG[provider_name]["api_keys"]:
                            current_llm_provider[0] = provider_name
                            speak_response(f"Switched to {current_llm_provider[0]} as the AI model, sir.")
                            bot_message = {"text": f"Switched model to {current_llm_provider[0]}.", "sender": "bot", "timestamp": datetime.datetime.now()}
            ...
            if any(phrase in lower_command for phrase in ["what time is it", "what is the time", "what's the time now", "current time"]):
                time_text = tell_current_time()
                chat_history.append({"text": time_text, "sender": "bot", "timestamp": datetime.datetime.now()})
                save_chat_history(chat_history)
                continue

            if handle_app_command(lower_command):
                chat_history.append({"text": f"Attempted to open application for command: {command}", "sender": "bot", "timestamp": datetime.datetime.now()})
                save_chat_history(chat_history)
                continue

            if set_alarm_or_reminder(lower_command):
                chat_history.append({"text": f"Set reminder for: {command}", "sender": "bot", "timestamp": datetime.datetime.now()})
                save_chat_history(chat_history)
                continue

            if handle_youtube_request(lower_command):
                chat_history.append({"text": f"Handled YouTube request for: {command}", "sender": "bot", "timestamp": datetime.datetime.now()})
                save_chat_history(chat_history)
                continue

            if take_and_save_photo(lower_command):
                chat_history.append({"text": f"Handled photo capture request for: {command}", "sender": "bot", "timestamp": datetime.datetime.now()})
                save_chat_history(chat_history)
                continue

            if perform_chrome_search(lower_command, provider_store=current_llm_provider):
                chat_history.append({"text": f"Performed web search for: {command}", "sender": "bot", "timestamp": datetime.datetime.now()})
                save_chat_history(chat_history)
                continue
            ...
            if "what do you see" in lower_command or "analyze image" in lower_command or "look around" in lower_command:
                ...
                if image_data_base64:
                    ...
                    chat_history.append({"text": vision_response, "sender": "bot", "timestamp": datetime.datetime.now()})
                else:
            ...
            if not classified_commands:
                speak_response("Sir, I couldn't classify your request with the decision-making model. I will answer it as a general query.")
                response_text, used_provider = call_llm_api_with_fallback(current_llm_provider[0], chat_history, command)
                if used_provider != current_llm_provider[0]:
                    current_llm_provider[0] = used_provider
                    speak_response(f"Sir, I have automatically switched to {used_provider} because the previous model failed.")
                chat_history.append({"text": response_text, "sender": "bot", "timestamp": datetime.datetime.now()})
                save_chat_history(chat_history)
                continue
            ...
            for classified_cmd in classified_commands:
                if classified_cmd.startswith("exit"):
                    ...
                    # If it's a general query, send to the current LLM with automatic fallback
                    response_text, used_provider = call_llm_api_with_fallback(current_llm_provider[0], chat_history, query_for_llm)
                    if used_provider != current_llm_provider[0]:
                        current_llm_provider[0] = used_provider
                        speak_response(f"Sir, I have automatically switched to {used_provider} because the previous model failed.")
